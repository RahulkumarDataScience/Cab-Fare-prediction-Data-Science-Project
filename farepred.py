# -*- coding: utf-8 -*-
"""
Created on Fri May 10 16:05:50 2019
...
sns.scatterplot(x="pickup_longitude", y="fare_amount", data= train, palette="Set2")
sns.scatterplot(x="pickup_latitude", y="fare_amount", data= train, palette="Set2")
sns.scatterplot(x="pickup_latitude", y="pickup_longitude", data= train, palette="Set2")
sns.scatterplot(x="dropoff_latitude", y="dropoff_longitude", data= train, palette="Set2")


##################################################  Density Plots ##################################################################################################

sns.kdeplot(train['fare_amount'], shade = True)
sns.kdeplot(train['passenger_count'], shade = True)
sns.kdeplot(train['month'], shade = True)
sns.kdeplot(train['day'], shade = True)
sns.kdeplot(train['weekday'], shade = True)
sns.kdeplot(train['hour'], shade = True)
sns.kdeplot(train['dropoff_latitude'], shade = True)
sns.kdeplot(train['pickup_longitude'], shade = True)
sns.kdeplot(train['pickup_latitude'], shade = True)
sns.kdeplot(train['dropoff_longitude'], shade = True)
...
svr_model = SVR(C= 100, gamma = 0.001).fit(X_train , y_train)
...
dt_model = tree.DecisionTreeRegressor(max_depth =5).fit(X_train, y_train)
...
gbr_model = GradientBoostingRegressor(max_depth= 2,learning_rate = 0.1).fit(X_train , y_train)
...
rf_model = RandomForestRegressor(max_depth= 10, n_estimators = 200).fit(X_train , y_train)
...
df = pd.DataFrame({"rmse":[3.97,3.98,3.98,3.99,4.03,4.01,4.01,3.91],\
